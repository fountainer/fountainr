...
#' @param progress Create a progress bar("none", "txt", "tk", "win", "timer")
...
roll_apply <- function(x, n, fun, ...,progress = "none", SIMPLIFY = FALSE) {
  fun <- match.fun(fun)
  n_row <- nrow(x)
  if (n_row < n ) {
  ...
  op <- pboptions(type = progress)

  res <- pblapply(vec_seq, function(i) fun(x[i:(i + n - 1), ], ...))
  if (SIMPLIFY) {
